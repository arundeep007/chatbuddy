...
class SignUpFragment : BaseFragment() {
    ...
    private fun checkValidations() {
        ...
        when(true)
        {
            ...
            confirmPassword.isEmpty()-> AppUtils.showToast("Confirm Password is Mandatory!",context)
            else -> {

                when(true)

                {
                    (username.length<=6) -> AppUtils.showToast("Username length should be greater than 6 !",context)
                    ...
                    (password!=confirmPassword)->AppUtils.showToast("Password and confirm Password are equal!",context)
                    else -> {
            ...
            }else
            {
                ...
                when(true)
                {
                    message.contains("The email address is already")->
                        AppUtils.showToast("Email is already in use. Please sign in or reset your password.",context)
                    else -> {
